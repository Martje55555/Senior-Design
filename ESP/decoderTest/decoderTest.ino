...
void setup() {
  ...
  pinMode(D3, OUTPUT); // Power for sensor DHT-11
  pinMode(D4, OUTPUT); // Power for sensor FC-28
  pinMode(D5, OUTPUT); // Powers Decoder
  ...
  if (WiFi.status() == WL_CONNECTED) {
    ...
    for (int i = 0; i < 11; i++) {
      if (i == 0) {
        ...
        doc["dht_hum"] = dataStorage[i];
      }
      else if (i == 10) {
        doc["dht_temp"] = dataStorage[i];
      }
    }
...
void sensorLoop()
{
  digitalWrite(D5, HIGH); // //Powers Decoder
  for (int i = 0; i < 8; i++) //
  {
    digitalWrite(D0, LOW);
    digitalWrite(D1, LOW);
  ...
  digitalWrite(D4, HIGH); //Turn on FC-28
  delay(1000);
  output_value0 = analogRead(sensor_pinA0);
  output_value0 = map(output_value0, 550, 0, 0, 100);
  dataStorage[8] = output_value0;
  digitalWrite(D4, LOW); //Turn off FC-28
  ...
  delay(1000);
  digitalWrite(D3, HIGH); //Turn on DHT-11
  dht_sensor.read11(sensor_pinA0);
  dataStorage[9] = dht_sensor.humidity;
  dataStorage[10] = dht_sensor.temperature;
  digitalWrite(D3, LOW); //Turn off DHT-11
